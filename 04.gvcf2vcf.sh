...
# 84.SNP.filtered.PASS.vcf.gz
TMPDIR=$WD/tmp
mkdir $TMPDIR
# tabix 84.${filename}.vcf.gz 
java -Xms80g -Xmx120g -Djava.io.tmpdir=$TMPDIR -jar $gatk SelectVariants --exclude-non-variants \
       -R ${REF} \
       -V 84.${filename}.vcf.gz \
       -O 84.${filename}.novariants.wms.vcf.gz &

java -Xms80g -Xmx120g -Djava.io.tmpdir=$TMPDIR -jar $gatk SelectVariants --exclude-non-variants \
       -R ${REF} \
       -V 84.${filename}.vcf.gz \
       -O 84-81.${filename}.novariants.wms.vcf.gz \
       --sample-name 81sample.list 
